    ...
    describe('Functionality', () => {
        ...
        describe('Request reward', () => {
            beforeEach(async () => {
                ...
                authorization = await deployMockContract(
                    deployer,
                ...
                requestsContract = await new Requests__factory(deployer).deploy(
            ...
            it('Should revert when not registered user makes request', async () => {
                ...
                await expect(
                    requestsContract
                        .connect(address)
                        .request(Wallet.createRandom().address)
                ).to.be.reverted;
            });

            it('Should revert when activity is inactive', async () => {
                const activity = await deployMockContract(
                    deployer,
                ...
                await expect(
                    requestsContract.request(activity.address)
                ).to.be.revertedWith('Requests__ActivityInactive');
            });

            it('Should create request', async () => {
                expect(
                    await requestsContract.getRequests(BigNumber.from(0))
                ).to.have.lengthOf(0);
            ...
            it('Should emit event after creating request', async () => {
                ...
                await expect(
                    requestsContract.request(activity.address)
                ).to.emit(requestsContract, 'RequestAdded');
            });
        });
        ...
        describe('Review reward', () => {
            beforeEach(async () => {
                ...
                const activity2 = await deployMockContract(
                    deployer,
                    Activity__factory.abi
                );
                ...
                const activity3 = await deployMockContract(
                    deployer,
                    Activity__factory.abi
                );
            ...
            it('Should change status to approved when approved', async () => {
                ...
                expect(
                    await requestsContract.getRequests(approvedStatus)
                ).to.have.lengthOf(0);
                await requestsContract.review(0, approvedStatus);
                expect(
                    await requestsContract.getRequests(approvedStatus)
                ).to.have.lengthOf(1);
                expect(await requestsContract.getRequest(0)).to.haveOwnProperty(
                    'status',
                    approvedStatus
                );
            });
            ...
            it('Should change status to rejected when rejected', async () => {
                ...
                expect(
                    await requestsContract.getRequests(rejectedStatus)
                ).to.have.lengthOf(0);
            ...
            it('Should emit event when reviewed', async () => {
                ...
                await expect(
                    requestsContract.review(0, approvedStatus)
                ).to.emit(requestsContract, 'RequestReviewed');
            });
        });
    });
});
