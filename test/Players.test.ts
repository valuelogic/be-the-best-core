...
describe('Players contract', async () => {
    ...
    describe('Players list', () => {
        it('Should add players', async () => {
            ...
            await playersContract
                .connect(playerManager)
                .addPlayer(playerWallet1.address, nick);
            ...
            await playersContract
                .connect(playerManager)
                .addPlayer(playerWallet2.address, 'random nick');
        ...
        it('should be reverted when added players wallet address have been already used', async () => {
            await playersContract
                .connect(playerManager)
                .addPlayer(playerWallet1.address, 'nick1');

            await expect(
                playersContract
                    .connect(playerManager)
                    .addPlayer(playerWallet1.address, 'nick2')
    ...
    describe('Add points', () => {
        it('Should add points to player', async () => {
            await playersContract
                .connect(playerManager)
                .addPlayer(playerWallet1.address, 'nick');
            const player = await playersContract.getPlayer(
                playerWallet1.address
            );
            expect(player.points).to.equal(0);
            ...
            const playerWith2Points = await playersContract.getPlayer(
                playerWallet1.address
            );
            ...
            await playersContract
                .connect(requestManager)
                .addPoints(playerWallet1.address, 3);
            const playerWith5Points = await playersContract.getPlayer(
                playerWallet1.address
            );
        ...
        it('should be reverted when player not exists', async () => {
            await expect(
                playersContract
                    .connect(requestManager)
                    .addPoints(playerWallet1.address, 4)
    ...
    describe('substract points', () => {
        it('should substract players points', async () => {
            ...
            await playersContract
                .connect(requestManager)
                .addPoints(playerWallet1.address, 7);
            const playerWith2Points = await playersContract.getPlayer(
                playerWallet1.address
            );
            ...
            await playersContract
                .connect(requestManager)
                .substractPoints(playerWallet1.address, 3);
            const playerWith5Points = await playersContract.getPlayer(
                playerWallet1.address
            );
        ...
        it('should store 0 points when substract more points then player currently has', async () => {
            ...
            await playersContract
                .connect(requestManager)
                .substractPoints(playerWallet1.address, 8);
            const playerWith5Points = await playersContract.getPlayer(
                playerWallet1.address
            );
        ...
        it('should be reverted when player not exists', async () => {
            await expect(
                playersContract
                    .connect(requestManager)
                    .substractPoints(playerWallet1.address, 4)
    ...
    describe('SetNick', () => {
        it('should change players nick when triggered by admin', async () => {
            ...
            const result = await playersContract.getPlayer(
                playerWallet1.address
            );
        ...
        it('should change players nick when triggered by player himself', async () => {
            ...
            await playersContract
                .connect(playerWallet1)
                .setNick(playerWallet1.address, 'new_nick');
            const result = await playersContract.getPlayer(
                playerWallet1.address
            );
            expect(result.nick).to.equal('new_nick');
        ...
        it('should be reverted when action performed by non admin account and not a player himself', async () => {
            ...
            await expect(
                playersContract
                    .connect(playerWallet2)
                    .setNick(playerWallet1.address, 'nick')
