...
import {Authorization, Players} from "../typechain-types";
import {PLAYER_MANAGER, REQUEST_MANAGER} from "../utils/roles";

describe("Players contract", async () => {
  let adminWallet: SignerWithAddress,
    playerWallet1: SignerWithAddress,
    playerWallet2: SignerWithAddress,
    requestManager: SignerWithAddress,
    playerManager: SignerWithAddress;
  let playersContract: Players;

  const player = (await ethers.getSigners())[1];

  beforeEach(async () => {
    [adminWallet, playerWallet1, playerWallet2, requestManager, playerManager] = await ethers.getSigners();
    ...
    await authorization.grantRole(PLAYER_MANAGER, playerManager.address);
    await authorization.grantRole(REQUEST_MANAGER, requestManager.address);
  });
  ...
  describe("Players list", () => {
    it("Should add players", async () => {
      ...
      await playersContract.connect(playerManager).addPlayer(playerWallet1.address, nick);
      ...
      await playersContract.connect(playerManager).addPlayer(playerWallet2.address, "random nick");
    ...
    it("should be reverted when added players wallet address have been already used", async () => {
      await playersContract.connect(playerManager).addPlayer(playerWallet1.address, "nick1");

      await expect(
        playersContract.connect(playerManager).addPlayer(playerWallet1.address, "nick2")
  ...
  describe("Add points", () => {
    it("Should add points to player", async () => {
      ...
      await playersContract.connect(requestManager).addPoints(playerWallet1.address, 2);
      const playerWith2Points = await playersContract.getPlayer(playerWallet1.address);
      expect(playerWith2Points.points).to.equal(2);

      await playersContract.connect(requestManager).addPoints(playerWallet1.address, 3);
    ...
    it("should be reverted when player not exists", async () => {
      await expect(
        playersContract.connect(requestManager).addPoints(playerWallet1.address, 4)
      ).to.be.revertedWith("Players__AccountNotRegistered"); // TODO: Check arguments
    });

    it("should be reverted when action performed by non player_manager player", async () => {
      await playersContract.connect(playerManager).addPlayer(playerWallet1.address, "nick1");
      await expect(
          playersContract
          .connect(player)
  ...
  describe("substract points", () => {
    it("should substract players points", async () => {
      ...
      await playersContract.connect(requestManager).addPoints(playerWallet1.address, 7);
      const playerWith2Points = await playersContract.getPlayer(playerWallet1.address);
      expect(playerWith2Points.points).to.equal(7);

      await playersContract.connect(requestManager).substractPoints(playerWallet1.address, 3);
      const playerWith5Points = await playersContract.getPlayer(playerWallet1.address);
    ...
    it("should store 0 points when substract more points then player currently has", async () => {
      ...
      await playersContract.connect(requestManager).substractPoints(playerWallet1.address, 8);
      const playerWith5Points = await playersContract.getPlayer(playerWallet1.address);
    ...
    it("should be reverted when player not exists", async () => {
      await expect(
        playersContract.connect(requestManager).substractPoints(playerWallet1.address, 4)
